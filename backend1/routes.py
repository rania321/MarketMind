...
# Initialize the summary model (add with other model initializations)
...
def generate_summary():
    ...
    try:
        def generate():
            ...
            if not sentiment_data:
                raise ValueError("No sentiment analysis found for this product")
            return summary_model.generate_summary(sentiment_data)
        
        # Increase timeout to 60 seconds
        logging.info("Starting summary generation with 60s timeout")
        summary_result = func_timeout(60, generate)
        logging.info("Summary generation completed")
        
        if summary_result.get("status") == "failed":
            logging.error(f"Summary generation failed: {summary_result.get('error')}")
        ...
        logging.error("Summary generation timed out after 60 seconds")
        return jsonify({
            "success": False,
